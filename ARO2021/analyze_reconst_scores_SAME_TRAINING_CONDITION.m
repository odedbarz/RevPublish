%
% analyze_reconst_scores_SAME_TRAINING_CONDITION.m
%
% Description:
...
trainDRR = 3;
aux.cprintf('Keywords', '-> trainDRR: %s (%d)\n\n', drr.labels{trainDRR}, trainDRR);

if strcmpi('MUA', finfo.type)    
    % BINWIDTH : 1 ms
    % CAUSALITY: No
    %{
    finfo.n_neurons  = [1 5 10] ;% [10 25 50 100 150];  	% # of units to load from existing files
    %finfo.trainDRR   = trainDRR;
    finfo.date       = '09-Aug-2020';        
    finfo.path       = '../.data/reconstruct/MUA_(09-Aug-2020)_bw(1)ms_algo(ASD)_lags(40)ms_cau(0)_/';
    fn_emplate       = 'reconstruct_%s_(%s)_units(%d)_bw(1)ms_algo(asd)_fbands(30)_splits(12)_lags(40)ms_cau(0)_trainDRR(%d).mat';    
    %}
    
    % BINWIDTH : 5 ms
    % CAUSALITY: No
    % %{
    finfo.n_neurons  = 100;  	% # of units to load from existing files
    finfo.date       = '12-Oct-2020';        
    finfo.path       = '../.data/reconstruct/MUA_(12-Oct-2020)_units(100)_bw(5)ms_lags(30)ms_cau(0)_/';
    fn_emplate       = 'reconstruct_%s_(%s)_units(%d)_bw(5)ms_algo(regression)_fbands(30)_splits(12)_lags(30)ms_cau(0)_trainDRR(%d).mat';
    %}
...
finfo.fn = sprintf(fn_emplate, finfo.type, finfo.date, finfo.n_neurons(1), drr.dry);
warning 'off'
...
for n = 1:n_neuron_list
    ...
    % Load DRY condition:
    finfo.fn = sprintf(fn_emplate, finfo.type, finfo.date, finfo.n_neurons(n), drr.dry);
    aux.cprintf('g', '\n-> Loading <%s>...\n', finfo.fn);
    warning 'off'
    data_dry = load(fullfile(finfo.path, finfo.fn));
    warning 'on'
    n_neurons = data_dry.obj_list{1}.n_neurons;
    ...
    for jk = 1:n_JK 
        ...
        X_dry = spec_st.Sft{drr.dry}(:, jk == data_dry.splits.idx);    
        
        for rv = 1:n_drr 
            % Load SAME DRR condition:
            finfo.fn = sprintf(fn_emplate, finfo.type, finfo.date, finfo.n_neurons(n), rv);
            aux.cprintf('g', '\n-> Loading <%s>...\n', finfo.fn);
            warning 'off'
            data_same = load(fullfile(finfo.path, finfo.fn));
            warning 'on'

            % # of neurons
            assert( data_dry.obj_list{1}.n_neurons == data_same.obj_list{1}.n_neurons );
            n_neurons = data_dry.obj_list{1}.n_neurons;

            % Make sure that the loaded data and the requested data have the same
            % parameters
            assert(binwidth == data_same.spec_st.binwidth);
            assert(n_bands == data_same.spec_st.n_bands);
            assert(isnan(win_size_ms)&&isnan(data_same.spec_st.win_size_ms) || ...
                (win_size_ms == data_same.spec_st.win_size_ms));
            %assert(n_JK == data_same.splits.n_grps );
            assert( 1 == isequal(data_dry.splits, data_same.splits) );
            splits = data_dry.splits;
            % ==== END oF LOADING DATA_SAME ====
        
        
            % Cut out the testing chunk of the spectrogram
            % * n_JK == test_grp_number 
            X_est                 = data_dry.obj_list{rv,jk}.X_est;
            ...
            % For comparing between stimuli of various DRR conditions and the
            % reconstructed spectrograms
            X_est_same                    = data_same.obj_list{rv,jk}.X_est;
            X_drr                         = spec_st.Sft{rv}(:, jk == splits.idx);            
            gof3                          = goodness(X_drr, X_est);        
            scores_same_drr.CC(rv,jk,n)   = gof3.CC;
            scores_same_drr.mse(rv,jk,n)  = gof3.mse;
            scores_same_drr.nmse(rv,jk,n) = gof3.nmse;
            scores_same_drr.CCf(rv,jk,n,:)= gof3.CCf;
...
scores_same_drr.mu.CC = squeeze( median(scores_same_drr.CC, 2) );
scores_same_drr.SE.CC = squeeze( mad(scores_same_drr.CC,[],2)/sqrt(n_JK) );
scores_same_drr.mu.mse = squeeze( median(scores_same_drr.mse, 2) );
scores_same_drr.SE.mse = squeeze( mad(scores_same_drr.mse,[],2)/sqrt(n_JK) );
scores_same_drr.mu.nmse = squeeze( median(scores_same_drr.nmse, 2) );
scores_same_drr.SE.nmse = squeeze( mad(scores_same_drr.nmse,[],2)/sqrt(n_JK) );
...
analyze.filename = sprintf('Analysis_%s_(%s)_units(%d)_bw(%g)_fbands(%d)_trainDRR(1-5)',...
    finfo.type, date, n_neurons, binwidth, n_bands);
analyze.data_info = finfo;

save([analyze.path, analyze.filename], '-v7.3',...
    'binwidth', 'scores', 'scores_same_drr',...
    'spec_st', 'splits', 'n_neurons', 'CC_broadband_envelope',...
...
% plot(M./M(1,:), '.', 'MarkerSize', markersize);
ehnd = errorbar(repmat((1:drr.n_drr)', 1,2) + 0.01*[-1, 1], M./M(1,:),...
    errbar, '.', 'MarkerSize', markersize, 'LineWidth', 2.0);
hold off
...
disp(tbl_cc_env);
% writetable(tbl_cc_env, '../.data/tbl_cc_env.xls', 'WriteVariableNames', 1, 'WriteRowNames', 1);
...
xlabel('DRR');
ylabel('$CC(S_{DRR}(f,t)-to-\hat{S}_{DRR}(f,t))$');
...
CCf2_avg_intervals = squeeze( mean(scores_same_drr.CCf(drr_idx,:,n_neurons_idx,:), 2) );
...
spec_thr    = 40;  % (dB) threshold\remove from spectrogram before plotting it
color_axis  = [0, 45]; 
fontsize    = 24;
