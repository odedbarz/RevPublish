function [fcoefs, cf] = MakeERBFilters(fs, numChannels, lowFreq, highfreq, rabbit_flag)
...
if nargin < 4 || isempty(highfreq)
    highfreq = min(42e3, fs/2);
end

if nargin < 5   % 08/30-2018, Barzelay
    rabbit_flag = false;
end

T = 1/fs;
cf = ERBSpace(lowFreq, highfreq, numChannels, rabbit_flag);
...
else    % *** USE RABBIT'S COCHLEA PARAMETERS ***
    ERB = rabbitERB(cf);
