function done=imp_gen
%
% IMP_GEN
%
%	IMP_GEN is called by the function RM_IMP.
...
%
%	All variables are passed as global variables from RM_IMP.
...
% 
% Global variables from RM_IMP
%
 global h src rec s_locations s_reflects fgains nfreq
 global sph_center sph_rad fs c taps ctap ctap2 
...
% and insert into overall impulse response
 ...
 for kk = 1:num_rec, 
  ...
  % Calculate samples delay for source to current receiver
   thit = 11+sr_dist/c*fs;	% number of samples delay to travel to rec
  ...
  % For each source location, determine the impulse response.
   h_temp = 1./(sr_dist(v)*ones(1,2*ctap-1+2*ctap2-1-1)).* ...
	 (shpfilt(fhit(v),nfreq,gains(v,:),th(v),rel_direct(v), ...
