function hout = shpfilt(sdelay,freq,gain,th,rel,a,r,fs,ctap,ctap2);
%
% SHPFILT	shaped filter design for non-integer delay
%
%               SHPFILT(SDELAY,FREQ,GAIN,TH,REL,A,R,CTAP,CTAP2) is to
%		be used by IMP_GEN to generate the
...
%
 ...
 if ctap2>1,
  ...
  %
   G(:,17)=real(G(:,17));
   G=[G,fliplr(conj(G(:,2:16)))];    	% Transform into appropriate
