...
lags_ms_list = [5, 10, 20, 30, 40, 50];
n_lags    = length(lags_ms_list);
...
aux.cprintf('string', '--> iscausal    : %d\n', iscausal);
aux.cprintf('string', '--> n_lags      : %d\n', n_lags);
...
clear gof
gof.CC  = nan(n_splits, n_lags);
gof.mse = nan(n_splits, n_lags);
gof.nmse= nan(n_splits, n_lags);


for nn = 1:n_lags
    % Training responses
    y1 = squeeze( H_unit(:, train_drr, slc.optimal_sorted) );
    aux.vprint(verbose, '--> lags_ms: %g\n', lags_ms_list(nn));
    ...
    for sp = 1:n_splits
        ...
        % *** Test *** 
        % Testing responses
        y2 = squeeze( H_unit(:, test_drr, slc.optimal_sorted) );
...
if ~isempty(fignum)
    ...
    figure(23+fignum);
    %clf;
    ax = errorbar(0.5+lags_ms_list, mean(gof.CC,1), std(gof.CC,[],1), ':.');
    set(ax, 'MarkerSize', 24, 'LineWidth', 2);    
    xlabel('Lags (ms)');
    %xlim([0.5, 0.5+lags_ms_list(end)]);
    %ylim([0.3, 1.0])
    ylabel('CCs');
    title(sprintf('iscausal: %d, binwidth: %g ms', iscausal, binwidth));
    set(gca, 'FontSize', 24);
    
    
    figure(25+fignum);
    clf;
    dot_pars = plot_dotbox(gof.CC, 'labels', lags_ms_list);
    ax = dot_pars.ax;
    xlabel('Lags (ms)');
    ylabel('CCs');
    title(sprintf('iscausal: %d, binwidth: %g ms', iscausal, binwidth));
    ...
    figure(27+fignum);
    clf;
    plot(lags_ms_list, gof.CC([1,4,7,12], :)', '.:', 'MarkerSize', 24);
