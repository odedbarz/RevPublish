%
% main_create_cc_database_trainDRRs.m
%
% Description:
...
fn_path     = fullfile(fn_path, 'Reconstruct_sortType(SPK-RND)_units(100)_(21-Jan-2022)_multiruns_trainDRRs');

fn_template = '1_reconstruct_%s_(20-Jan-2022)_units(100)_train(%d)_bw(5)ms_type(SPK-RND)_fbands(30)_lags(30)ms';

train_drrs = [1:5];


n_train_drrs = length(train_drrs);
...
%% Start Looping over all train_drrs
for un = 1:n_train_drrs
    % Loading first file to get preliminary metadata
    %       splits: [1×1 struct]
    %      spec_st: [1×1 struct]
    %      stim_st: [1×1 struct]
    %     tbl_data: [241×20 table]
    fn_n = sprintf(fn_template, data_type, train_drrs(un));
    ...
    for sp = 1:n_splits 
        ...
        for k = 1:n_drr    
            ...
            if debug_flag
                ...
                xlabel('Time (sec)');
                ylabel('CC');
                title(sprintf('%s %d', data_type, train_drrs(un)));
            end
        end
    end
    ...
    fn_name = sprintf('analyzed_%s_CC_(%s)_trainDRRs(%d).mat', data_type, date, train_drrs(un));
