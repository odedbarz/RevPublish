...
%%
fignum = 10;
n_stats = 100;        % # of calculations to set the significance

% ENVELOPE_ALGO_TYPE, sets the way to calculate the envelope
% (1) Use the neuron's BF(STRF) to extract the desired envelope;
% (2) Perform convolution between unit response and stimulus spectrogram and get the 
%     best correlated band;
% (3) Random envelope. In this case, use N_STATS. Otherwise, set N_STATS = 1.
envelope_algo_type = 3;

if 3 ~= envelope_algo_type
    n_stats = 1;
end
...
% Average
if 3 ~= envelope_algo_type || ~isfield(scores, 'mu')
    scores.mu.CCer = nan(1, n_drr);       % % CC(response dry & response)
    scores.mu.MDr  = nan(1, n_drr);
    scores.mu.MG   = nan(1, n_drr);
    scores.mu.skew = nan(1, n_drr);
    scores.mu.ku   = nan(1, n_drr);
end

% The p-values
if 3 == envelope_algo_type || ~isfield(scores, 'pvalues')
    scores.pvalue.CCer = nan(n_stats, n_drr);       % % CC(response dry & response)
    scores.pvalue.MDr  = nan(n_stats, n_drr);
    scores.pvalue.MG   = nan(n_stats, n_drr);
    scores.pvalue.skew = nan(n_stats, n_drr);
    scores.pvalue.ku   = nan(n_stats, n_drr);
end


for pv = 1:n_stats
    for n = 1:n_units
        switch envelope_algo_type
            case 1
                % Option #1:
            ...
            otherwise
                error('--> Unrecognized envelope_algo_type value!');
        end
    ...
    % The p-values
    if 3 == envelope_algo_type
        scores.pvalue.CCer(pv,:)= mean(scores.CCer);  
        scores.pvalue.MDr(pv,:) = mean(scores.MDr);  
        scores.pvalue.MG(pv,:)  = mean(scores.MG);  
        scores.pvalue.skew(pv,:)= mean(scores.skew);  
        scores.pvalue.ku(pv,:)  = mean(scores.ku);   
    else
        scores.mu.CCer(pv,:)= mean(scores.CCer);  
        scores.mu.MDr(pv,:) = mean(scores.MDr);  
        scores.mu.MG(pv,:)  = mean(scores.MG);  
        scores.mu.skew(pv,:)= mean(scores.skew);  
        scores.mu.ku(pv,:)  = mean(scores.ku);   
    end
end



%% Plot the results -- You need to run this script TWICE!
% First, set 3 ~= envelope_algo_type, 
% then set 1 == envelope_algo_type.
...
D2 = scores.mu.CCer(:,drr_idx);
color = 'k';
pars = plot_dotbox(D2, 'labels', drr.labels(drr_idx), 'color', color);
h = legend(pars.ax, 'mean over real BFs');

hold on
D = scores.pvalue.CCer(:,drr_idx);
color = '';
plot_dotbox(D, 'labels', drr.labels(drr_idx), 'color', color);
hold off
set(h, 'String', 'mean over real BFs');

% 
% if 3 ~= envelope_algo_type
%     D = mean(scores.CCer(:,drr_idx));
%     hold on;
%     color = 'k';    
% else
%     D = scores.pvalue.CCer(:,drr_idx);
...
% pars = plot_dotbox(D, 'labels', drr.labels(drr_idx),...
%     'color', color);
