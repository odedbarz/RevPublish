...
fn.load = fullfile('../.data/', 'data_MUA_(08-Jun-2020)_bw(10)_fbands(30)_win(5)ms_spec(gammatone).mat');
aux.vprint(verbose, '--> [main_reconstruction_PSTH.m]: Loading <%s> ...\n', fn.load);
load(fn.load);


'^%^%%%$ DEBUG ^^%^#$ !!!'
%neurons = randperm(size(H,3), 25);
neurons = neu_cfinv;
%         neurons = [neurons, randperm(size(H,3), 25)];     '^%^%%%$ DEBUG ^^%^#$ !!!'
H = H(:,:,neurons);
