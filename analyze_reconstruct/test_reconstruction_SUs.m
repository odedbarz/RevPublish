...
fn.load = fullfile('../.data', 'data_PSTH_bw(2)_fbands(30)_win(20)ms_(07-May-2020).mat');
aux.vprint(verbose, '--> [main_reconstruction_PSTH.m]: Loading <%s> ...\n', fn.load);
load(fn.load);


        '^%^%%%$ DEBUG ^^%^#$ !!!'
        neurons = randperm(size(H,3), 20);
        H = H(:,:,neurons);
...
inv_type = [2, 0.00001];    % [2, 0.005]

% Initialize the reconstruction object
obj = reconstruct(binwidth, 'inv_type', inv_type); 
...
%{
    'SAVE the analysis!'
    fn.save = sprintf('../.data/reconstruct_PSTH_bw(%g)_fbands(%d)_win(%g)ms_(%s).mat',...
